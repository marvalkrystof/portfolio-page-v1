...
function SidebarNavButton(props : NavButtonProps)  {
  ...
  if(isVisible) {
    ...
    props.targetElement.current?.classList.remove("bg-content-background-hover")
  }

  return (
    <>
   <button className={`text-start text-base pl-3 pr-3 pt-1 pb-1 mt-5 border-l-2 rounded border-gray-500 text-white transition-transform duration-200
      ${isVisible ? 'translate-x-10 text-opacity-100 border-white bg-content-background-hover' : 'text-opacity-30'}
